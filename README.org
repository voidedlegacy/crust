...
** Language
- Statements:
  - =struct Name { field: type; ... }= (or =struct Name:= ... =end=)
  - =fn name(args) -> type { ... }= (or =fn name(args) -> type:= ... =end=)
  - =let name = expr=
  - =set name = expr=
- =use "path"= or =use "path" as name= (file paths)
- =use std::io= or =use foo::bar= (module paths)
  - =mod name=
  - =print expr, expr, ...=
  - =return expr=
  - if expr: ... else: ... end
...
- Expressions:
  ...
  - field access: =p.x=
  - type names: =int=, =str=, =bool=, or a struct name
...
=std::string= modules available. Some functions are implemented in Crust
directly (see =std/math.crust=), while others are built-ins.
